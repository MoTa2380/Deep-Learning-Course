...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dw = torch.matmul(x_reshaped.t(), dout)
...
def softmax_loss(x, y):
    ...
    ######################################################################
    # Replace "pass" statement with your code
    shifted_logits = x - torch.max(x, axis=1, keepdims=True).values
    Z = torch.sum(torch.exp(shifted_logits), axis=1, keepdims=True)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        reg_loss += self.reg * torch.sum(self.params['W2']**2)
        loss = data_loss + reg_loss

        # Backward pass: compute gradients
        grads = {}
        dout1, grads['W2'], grads['b2'] = Linear.backward(dscores, cache2)
        dx, grads['W1'], grads['b1'] = Linear_ReLU.backward(dout1, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          self.params['W' + str(i+1)] = weight_scale * torch.randn(layers_dims[i], layers_dims[i+1]).to(device=device, dtype=dtype)
          self.params['b' + str(i+1)] = torch.zeros(layers_dims[i+1]).to(device=device, dtype=dtype)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
        ...
        self.dropout_masks = [None for _ in range(self.num_layers - 1)]
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            ...
            if i == self.num_layers:
                scores, cache[i] = Linear.forward(scores, W, b)
            else:
                scores, cache[i] = Linear_ReLU.forward(scores, W, b)
                if self.use_dropout:
        ...
        for i in range(1, self.num_layers + 1):
            ...
            reg_loss += self.reg * torch.sum(W**2)
        loss = data_loss + reg_loss
        ...
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
                dx, dw, db = Linear.backward(dscores, cache[i])
            else:
                if self.use_dropout:
                  dx = Dropout.backward(dx, dropout_caches[i-1])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict,
                ...
                batch_size=160,  # Increase batch size
                print_every=100)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
...
def adam(w, dw, config=None):
    ...
    next_w = w - learning_rate * mb / (torch.sqrt(vb) + epsilon)
